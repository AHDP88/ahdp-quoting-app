    ...
    <div className="min-h-screen" style={{ background: "#f5f5f0" }}>
      ...
      <section className="relative overflow-hidden bg-[#1f3d2b]">
        <div className="absolute inset-0 opacity-20"
          style={{ backgroundImage: "radial-gradient(circle at 20% 50%, #8b5a2b 0%, transparent 60%), radial-gradient(circle at 80% 20%, #2d6a4f 0%, transparent 60%), radial-gradient(circle at 50% 100%, #1a5c3a 0%, transparent 50%)" }}
        />
        <div 
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 bg-white/12 rounded-full px-4 py-1.5 text-white/80 text-sm font-medium mb-6">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-tight leading-tight mb-5">
              Deck & Pergola<br />
              <span className="text-green-300">Quote Builder</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <a href="#" onClick={e => e.preventDefault()} className="inline-flex items-center justify-center gap-2 bg-white/15 hover:bg-white/20 text-white font-medium text-sm px-6 py-3 rounded-xl transition-all border border-white/35 cursor-pointer">
      ...
      <div className="h-1 bg-gradient-to-r from-[#1f3d2b] via-[#8b5a2b] to-[#1f3d2b]" />

      {/* Features */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {features.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-white rounded-xl p-6 border border-[#1f3d2b]/8 border-l-[3px] border-l-[#1f3d2b]/15 shadow-sm">
              <div className="w-10 h-10 rounded-xl bg-[#1f3d2b]/12 flex items-center justify-center mb-4">
                <Icon className="h-5 w-5 text-[#1f3d2b]" />
              </div>
              <h3 className="font-semibold text-[#1f3d2b] mb-1.5">{title}</h3>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center">
            ...
            <span className="ml-2 text-xs font-medium bg-[#1f3d2b]/8 text-[#1f3d2b] px-2 py-0.5 rounded-full">6</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {mockQuotes.slice(0, 6).map((quote) => {
            ...
            return (
              <a key={quote.id} href="#" onClick={e => e.preventDefault()} className="block bg-white rounded-xl p-5 border border-gray-200/80 shadow-sm hover:shadow-md hover:border-[#1f3d2b]/30 transition-all cursor-pointer group">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="font-bold text-gray-900 group-hover:text-[#1f3d2b] transition-colors">{quote.clientName}</h3>
                    <span className="inline-block text-[10px] font-medium bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded tracking-wide">{projectLabel}</span>
                  </div>
                  <ChevronRight className="h-4 w-4 text-gray-300 group-hover:text-[#1f3d2b] transition-colors mt-0.5" />
                </div>
                <div className="flex items-end justify-between">
                  ...
                  {totalIncGST !== null && (
                    <div className="text-right">
                      <p className="text-xl font-bold text-[#1f3d2b]">{formatCurrency(totalIncGST)}</p>
